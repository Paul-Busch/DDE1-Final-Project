...
class GreedyShapeletSearch():
   ...
   # n=1 max=20
    def get_top_k_shapelets(self, X_train, y_train, n_shapelets=5, shapelet_min_size = 10, shapelet_max_size=25):

        for i in range(n_shapelets):
            print(f"Searching for shapelet {i+1}...")
            self.shapelets = []
            self.main_event_loop(X_train, y_train, shapelet_min_size = shapelet_min_size, shapelet_max_size = shapelet_max_size)
            print(f"Found shapelet {i+1} at sample: {self.top_shapelets[i][0]}, candidate: {self.top_shapelets[i][1]}.")
    ...
    def main_event_loop(self, X_train, y_train, shapelet_min_size=30, shapelet_max_size=31):
        """
        ...
        for shapelet_size in tqdm(range(shapelet_min_size, shapelet_max_size)):
            ...
            self.evaluate_candidates(profiles, y_train, shapelet_size)
        # Retrieve top shapelets
        self.retrieve_top_shapelet(X_train)
    
    def evaluate_candidates(self, profiles, y_train, shapelet_size):
        """
        ...
        for sample_idx in range(len(profiles)):
            ...
            for candidate_idx in range(sample.shape[0]):
                ...
                # Score candidate
                try:
                    score, margin = self.fit_svm(features, y_train)
